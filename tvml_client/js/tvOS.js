...
if (typeof console === "undefined" || typeof window === "undefined") {
  ...
  var liveScreen = { screenSize: func };
}
...
var tvOS = {
  ...
  // * tvOS.liveScreen
  // *
  // * Fake liveScreen object
  // *
  // * @var object liveScreen
  liveScreen: {
    // Just 'fake'.
  },
    ...
    if (typeof list === "object") {
      for (var i = 0; i < list.length; i++) {
        ...
        if (typeof list[i]["image_height"] === "undefined")
          list[i]["image_height"] = 500;
        if (typeof list[i]["image_width"] === "undefined")
          list[i]["image_width"] = 500;
   ...
   * @example tvOS.screenSize(20-20-400-400)
   */
  screenSize: function (size) {
    if (typeof liveScreen === "object") {
      // Used tvOS app with hack.
      this.dismiss();
      // Set a timeout (due lag of dismiss)
      setTimeout(
        function (size) {
          liveScreen.screenSize(size);
        },
        1000,
        size
      );
